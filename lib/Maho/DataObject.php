...
class DataObject implements ArrayAccess, JsonSerializable
{
    ...
    protected function __toXml(array $arrAttributes = [], $rootName = 'item', $addOpenTag = false, $addCdata = true)
    {
        ...
        $xmlModel = new Simplexml\Element('<node></node>');
     ...
     * with special handling for nested DataObject instances:
     *
    ...
    public function jsonSerialize(): array
    {
        ...
        foreach ($data as $key => $value) {
            if ($value instanceof DataObject) {
                ...
                if ($reflection->getMethod('jsonSerialize')->getDeclaringClass()->getName() !== \Maho\DataObject::class) {
                    continue;
                }

                // If the object has a custom toJson method, use it to convert to JSON string
                if ($reflection->getMethod('toJson')->getDeclaringClass()->getName() !== \Maho\DataObject::class) {
