...
class Pgsql extends AbstractPdoAdapter
{
    ...
    public function insertForce(string $table, array $bind): int
    {
        ...
        // Bump the backing sequence of any auto-incrementing column we just gave
        // an explicit value, so subsequent non-explicit inserts don't collide.
        // pg_get_serial_sequence resolves the sequence for both legacy SERIAL
        // columns (where the column default is "nextval(...)") and declarative
        // GENERATED BY DEFAULT AS IDENTITY columns (where the column default is
        // null and the sequence is attached via pg_depend).
        $tableInfo = $this->describeTable($table);
        foreach ($bind as $column => $value) {
            if (!isset($tableInfo[$column]) || empty($tableInfo[$column]['IDENTITY'])) {
                continue;
            }
