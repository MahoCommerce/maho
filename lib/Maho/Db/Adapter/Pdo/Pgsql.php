...
class Pgsql extends AbstractPdoAdapter
{
    ...
    /**
     * Advance the IDENTITY-backing sequence of every column in $bind we wrote
     * an explicit numeric value to. Used by both insert() and insertForce().
     *
     * pg_get_serial_sequence resolves the sequence for both legacy SERIAL
     * columns (column default is "nextval(...)") and declarative GENERATED BY
     * DEFAULT AS IDENTITY columns (column default is null, sequence attached
     * via pg_depend).
     */
