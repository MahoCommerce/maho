 ...
 * representation — these are exactly the differences the parity checker
 * (.github/schema-dump.php) ignores:
 *
 ...
 *
 * The equivalence applied here is the one the parity dump defines, so it never
 * hides a difference the dump would show. Methods mutate the passed tables in
 * place; both are built fresh per plan() (introspection / Collector).
 */
final class Canonicalizer
{
     ...
     * nothing for it. The parity dump compares indexes by columns and semantics,
     * never by name, so this is invisible to it and removes the rename churn the
     * Comparator would otherwise generate (every legacy index name differs from
     * DBAL's autogenerated one).
     */
    ...
    private static function reconcileColumns(Table $live, Table $target): void
    {
        foreach ($target->getColumns() as $targetColumn) {
            ...
            // (e.g. 22/0) that the declarative schema never sets and that the
            // parity dump ignores. Align them so DOUBLE(22,0) vs DOUBLE stops
            // re-emitting forever.
            ...
            // as IDENTITY (autoincrement flag, no default). The parity dump
            // treats both as the same "autoincrement column", so align the live
            // column to the target's identity form. This removes a per-column
