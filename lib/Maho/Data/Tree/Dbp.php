...
class Dbp extends Tree
{
     ...
     * @param Node|\Maho\DataObject $node
    ...
    public function move($node, $newParent, $prevNode = null): void
    {
        ...
        $data = [
            $this->_levelField => new Expr("{$this->_levelField} + '{$levelDisposition}'"),
            $this->_pathField  => new Expr("CONCAT('$newPath', RIGHT($this->_pathField, LENGTH($this->_pathField) - {$oldPathLength}))"),
        ];
        $condition = $this->_conn->quoteInto("$this->_pathField REGEXP ?", "^$oldPath(/|$)");

        $this->_conn->beginTransaction();

        $reorderData = [$this->_orderField => new Expr("$this->_orderField + 1")];
        try {
            if ($prevNode && $prevNode->getId()) {
                ...
                $select = $this->_conn->select()
                    ->from($this->_table, new Expr("MIN({$this->_orderField})"))
