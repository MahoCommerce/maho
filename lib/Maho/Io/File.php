...
class File extends \Maho\Io
{
     ...
     * @var string[]
     */
    public const ALLOWED_IMAGES_EXTENSIONS = ['webp', 'avif', 'jpg', 'jpeg', 'png', 'gif', 'bmp'];
    ...
    protected function _assertInsideCwd(string $filename): void
    {
        if (str_contains($filename, chr(0))
            || preg_match('#(^|[\\\\/])\.\.($|[\\\\/])#', $filename)
            || ($this->_cwd && \Maho\Io::getPathWithinDir($this->_cwd, $filename) === null)
        ) {
    ...
    public function ls($grep = null)
    {
        ...
        if ($dh = opendir($dir)) {
            while (($entry = readdir($dh)) !== false) {
                ...
                $list_item['mod_date'] = date(\Mage_Core_Model_Locale::DATETIME_FORMAT, filectime($fullpath));
