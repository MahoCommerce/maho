...
class ProductConfigure // Maho.Admin.Controller.ProductConfigurePopup
{
    ...
    compatibilityFlags = {
        /**
         * Magento 1.9 stores copies of product config forms in DOM and copies it to dialogs on demand. This causes issues with
         * duplicate element IDs and is generally bad design. This updated class still stores config templates in DOM along with
         * a stored FormData object, but by default it detaches the container div that stores state.
         */
     ...
     * ```
     * # TODO: add examples of non prefixed form values
     * ```
     *
     * To use, set `{ urlFetch, urlConfirm }` for getting / posting data to the backend:
     * ```js
     ...
     * ```
     *
     * The other method allows the admin user to configure multiple products to batch submit to the Maho backend. There are
     ...
     * ```
     *     # TODO: Add more examples of prefixed + flat POST data
     *     list[sidebar_wishlist][item][42][qty] = 2
     * ```
     *
     ...
     * ```js
     *     # app/design/adminhtml/default/default/template/customer/tab/wishlist.phtml
     *     productConfigure.addListType(', { urlSubmit: '<?= $this->getUrl("...") ?>' });
    ...
    /**
     * Internal function to restore form values to a confirmed block
     */
