...
it('completes a full card order through to the success page with reconciled totals', function (string $display, float $rate) {
    ...
    // in USD, and reconcile: base_grand_total * base_to_order_rate == grand_total.
    $order = latestOrder();
    ...
    // PayPal's API and assert every field matches what Maho stored. The settlement amount
    // (authorization/capture) is what PayPal will actually charge.
    ...
    expect($paypal['order_currency'])->toBe($display);
    expect(round($paypal['order_value'], 2))->toBe($grandTotal);             // order amount on PayPal
    expect($paypal['settlement_currency'])->toBe($display);
    expect(round($paypal['settlement_value'], 2))->toBe($grandTotal);        // amount PayPal will charge
})->with([
