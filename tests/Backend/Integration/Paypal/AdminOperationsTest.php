...
it('config model is instantiable via alias', function () {
...
it('can instantiate observer', function () {
    $observer = Mage::getModel('paypal/observer');
...
it('can instantiate shortcut button block', function () {
    $block = Mage::app()->getLayout()->createBlock('paypal/shortcut_button');
    expect($block)->toBeInstanceOf(Maho_Paypal_Block_Shortcut_Button::class);
});

it('can instantiate admin config blocks', function () {
    $testConn = Mage::app()->getLayout()->createBlock('paypal/adminhtml_system_config_testConnection');
    expect($testConn)->toBeInstanceOf(Maho_Paypal_Block_Adminhtml_System_Config_TestConnection::class);

    $webhook = Mage::app()->getLayout()->createBlock('paypal/adminhtml_system_config_registerWebhook');
