...
final class ProductProvider extends \Maho\ApiPlatform\Provider
{
     ...
     * back-office tokens (lossless read-modify-write), display equals base, or no
     ...
     * currency (the storefront does the same before rendering); back-office reads
     * keep base values and relabel `currency` to the base code. Two deliberate
     * exceptions stay in base currency: `cost` (back-office-only, never converted)
    ...
    private function getCollectionCacheKey(array $filters): string
    {
        ...
        ksort($keyData);
        // Back-office readers get unconverted base-currency DTOs, so they must
        // not share cached entries with public readers of the same store/currency.
        $scope = StoreContext::getStoreId() . '_' . $this->getCustomerGroupId() . '_' . $this->resolveCurrencyCode()
            . ($this->isBackOfficeReader() ? '_backend' : '');
