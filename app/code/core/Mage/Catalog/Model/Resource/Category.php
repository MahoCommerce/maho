...
class Mage_Catalog_Model_Resource_Category extends Mage_Catalog_Model_Resource_Abstract
{
    ...
    protected function _beforeDelete(\Maho\DataObject $object)
    {
        ...
        if ($parentIds) {
            ...
            $data = ['children_count' => new Maho\Db\Expr('children_count - ' . $childDecrease)];
    ...
    public function changeParent(
        ...
        $adapter->update(
            $table,
            ['children_count' => new Maho\Db\Expr('children_count - ' . $childrenCount)],
        ...
        $adapter->update(
            $table,
            [
                'path' => new Maho\Db\Expr('REPLACE(' . $pathField . ',' .
                    $adapter->quote($category->getPath() . '/') . ', ' . $adapter->quote($newPath . '/') . ')'),
                'level' => new Maho\Db\Expr($levelFiled . ' + ' . $levelDisposition),
            ],
    ...
    protected function _processPositions($category, $newParent, $afterCategoryId)
    {
        ...
        $bind = [
            'position' => new Maho\Db\Expr($positionField . ' - 1'),
        ];
        $where = [
        ...
        if ($afterCategoryId) {
            ...
            $select = $adapter->select()
                ->from($table, ['position' => new Maho\Db\Expr('MIN(' . $positionField . ')')])
